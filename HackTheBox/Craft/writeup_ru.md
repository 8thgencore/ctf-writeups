...
Но, мы знаем, что с помощью функции `eval()` в `python2` можно исполнять различные команды. Воспользуемся следующий пейлодом: 
...
Измется замечательная таблица *user*. Выведем поля таблицы:
...
Получаем инфорамацию о трех юзеро. Первый нам известен, остальные два нет. 



#### Stage 6

Попробуем авторизироваться с логином и паролем пользователей на https://gogs.craft.htb/. Расмотрев их профили, обнаруживаем у пользователя **gilfoyle** приватный репозиторий. В корне репозитория имеется папка `.ssh` (ай-яй-яй :) ). Вытаскиваем от туда приватный ключ и сохраняем в файл  **id_rsa_craft**.
...
Попадаем на машину под рутом. Вытаскиваем флаг: 
